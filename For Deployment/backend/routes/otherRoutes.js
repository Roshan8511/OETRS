...
router.post("/", protect, adminOnly, upload.single('image'), async (req, res) => {
  try {
    ...
    const { title, description, eventType, organizerName, startDate, endDate, location, timing, totalSeats, price } = req.body;

    if (!title || !eventType || !organizerName || !startDate || !endDate || !location || !timing || !totalSeats) {
